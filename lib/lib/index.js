...
const ownerNumber = "558291099267"
const ownerName = "Miku"
const cdd = 480000 // 8 minutos
const ammOff = "off"
const blocked = []

//_LISTA DE CRS
const cr = "Miku-BOT v12.4"
...
async function starts() {
	...
	tiringa.on('qr', () => {
		console.log(color('👆'), color(' Escanear o código acima para iniciar o Miku-BOT!'))
	})

	fs.existsSync('./tiringa.json') && tiringa.loadAuthInfo('./tiringa.json')
	tiringa.on('connecting', () => {
		start('2', 'Conectando o Miku-BOT...')
	})
	tiringa.on('open', () => {
...
//_FUCTION NOME DOS STICKERS
				...
				if (!packname) packname = 'Miku-BOT'; if (!author) author = 'Bot';	
...
║├► ${prefix}slot
║│_Caça níqueis sem premiação alguma kkk_
║├► ${prefix}caracoroa
║│_Caracoroa_
║├► ${prefix}level
...
tiringa.sendMessage(from, `https://github.com/jeankid/Miku-BOT.git` , text, {quoted: mek})
break
...
case 'pão':
if (isOwner) return reply('Oi Miku')
else {
...
//_ENVIA O VCARD
case 'Miku':
const sentMsg  = await tiringa.sendMessage(from, {displayname: "Miku️", vcard: vcard}, MessageType.contact)
break
                    case 'leaderboard':
				case 'lb':
                      ...
                      if (!q.includes('|')) return reply(`Você deve digitar o nome do pacote e criador dessa forma:\nComando: ${prefix}fign nome do pack|nome do autor\nExemplo: ${prefix}fign Pack nagatoro1|Italu`)
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
								exec(`webpmux -set exif ${addMetadata('Miku-BOT', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
                ...
                case 'figurinha':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Miku-BOT', authorname)} ${ran} -o ${ran}`, async (error) => {
...
case 'stk':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								console.log(`Error : ${err}`)
                                exec(`webpmux -set exif ${addMetadata('Miku-BOT', authorname)} ${rano} -o ${rano}`, async (error) => {
								fs.unlinkSync(media)
								reply(mess.error.stick)
							})
                      })
...
mentions('╭╾╼◐⚋ ༒ᴍᴇɴᴄɪᴏɴᴀʀ ᴛᴏᴅᴏs ༒⚋◑╾╼╮\n║➸'+todos+'╰╾╼◐⚋⚋ ༒ Miku ༒ ⚋⚋◑╾╼╯', members_id, true)
break

//_LIMPA TODOS OS CHATS
				case 'clearall':
					if (!isOwner) return reply('Só a Miku pode fazer isso')
...
if (vit == "derrota" ) {
var tes = "A vitória é da Miku-BOT"
}
if (vit == "empate" ) {
							...
							if (coordX != 'a1' && coordX != 'a2' && coordX != 'a3' &&
								...
								if (WinnerX()) {
									...
									if (isLevelingOn) {
										...
										tiringa.sendMessage(from, `🎉🎉 VITÓRIA DA MIKU-BOT 🎉🎉\n\n➣  PUNIÇÃO: ${randomTTTXP} XP 🔮`, text)
									} else {
										tiringa.sendMessage(from, `🎉🎉 VITÓRIA DA MIKU-BOT 🎉🎉`, text)
									}
